...
void ordenar(){ //Método do bubblesort
    for(int i = 0; i < cod - 1; i++){ //O(n)
        for (int j = 0; j < cod - i - 1; j++){ //O(n)
            if (strcmp(contats[j]->nome, contats[j + 1]->nome) > 0){
                contato *temp = contats[j];
                contats[j] = contats[j + 1];
                contats[j + 1] = temp;
            }
        }
    }
}
//resultado: O(n)*O(n) = O(n²)
